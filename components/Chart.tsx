...
type Props = { symbol: string; candles: Candle[]; ema9?: number[]; ema21?: number[]; ema50?: number[]; height?: number; };
...
export default function Chart({ symbol, candles, ema9, ema21, ema50, height = 540 }: Props) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(containerRef.current, {
      height,
      layout: { background: { type: 'solid', color: 'transparent' }, textColor: '#e6edf7' },
      ...
      crosshair: { vertLine: { color: 'rgba(78,161,255,.45)' }, horzLine: { color: 'rgba(78,161,255,.45)' } },
    });

    const candleSeries = chart.addSeries(CandlestickSeries, { upColor: '#31d0aa', downColor: '#ff5b6e', wickUpColor: '#31d0aa', wickDownColor: '#ff5b6e', borderVisible: false });
    const volSeries = chart.addSeries(HistogramSeries, { priceFormat: { type: 'volume' }, priceScaleId: '', color: 'rgba(155,176,207,.55)' });
  ...
  useEffect(() => {
    ...
    const vols = candles.map(c => ({ time: c.time as any, value: c.volume, color: c.close >= c.open ? 'rgba(49,208,170,.45)' : 'rgba(255,91,110,.45)' }));
